...
public class ClientHandler {
    ...
    private void showHelp() {
        sendMessage(
            "-w nick message - написать в лс\n" +
            "-cn newNick - сменить ник\n" +
            "-pass newPass - сменить пароль"
        );
    }

    private void changePassword() {
        String newPassword = message.split("\\s")[1];

        Connection connection = DBConnection.getConnection();
        try {
            PreparedStatement statement = connection.prepareStatement("UPDATE credentials SET password = ? WHERE login = ?;");
